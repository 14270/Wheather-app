...
const API_KEY = "d64fc4591bbfef08c67e81cc80083cad";
...
const formatToLocalTime = (
  secs,
  timezone = 0, // ✅ Default timezone to 0
...
const formatForecastWeather = (secs, timezone, data) => {
  if (!data || !data.list) return { hourly: [], daily: [] }; // ✅ Check for undefined `data.list`
  ...
  const daily = data.list
    .filter((f) => f.dt_txt?.slice(-8) === "00:00:00") // ✅ Added optional chaining `?.`
    .map((f) => ({
      temp: f.main.temp,
